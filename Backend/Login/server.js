...
app.get("/test", (req, res) => {
  res.status(200).json({ msg: "API test successful" });
});
  ...
  try {
    ...
    if (existingEmail) {
      if (!existingEmail.isEmailVerified) {
        ...
        // await sendOtpEmail(email, otp); // Disabled for initial account creation
...
app.post("/api/resend-otp", async (req, res) => {
  ...
  const { "forgot-password": forgotPassword } = req.query;
...
app.post("/api/sharedLink/count", async (req, res) => {
  const { username, shareId, title, expiryTime } = req.body;
...
app.post("/api/user/sharedLinks", cleanExpired, async (req, res) => {
  ...
  try {
    ...
    const currentDate = new Date();
    const expiredLinks = user.sharedLinks.filter(
      (link) => new Date(link.expiryTime) <= currentDate
    );
    ...
    const sharedLinksWithoutId = user.sharedLinks.map((link) => {
      const { _id, ...linkWithoutId } = link.toObject();
